...
namespace HISDataAccess
{
    ...
    ***************************************************************/
    ...
    public class DataHelper
    {
        ...
        public DataHelper(int WorkStationID, int DataBaseID,int intHospitalId=3)
        {
            string MasterVal = string.Empty;
            try
            {
                //below code section added for website key config
                //GetConKeyFromService(DataBaseID);
                if (connString == string.Empty)
                {
                    switch (DataBaseID)
                    {
                        case 1:
                            if(intHospitalId == 1)     
                                 MasterVal = "DBConnectionStringMasters1";
                            else if(intHospitalId == 2)
                                 MasterVal = "DBConnectionStringMasters2";
                            else
                                 MasterVal = "DBConnectionStringMasters";
                        ...
                        case 2:
                            ...
                            break;
                        case 3:
                            string ReportVal = "DBConnectionStringReports";
                            ConnectionStringSettings ReportConnstr = ConfigurationManager.ConnectionStrings[ReportVal];
        ...
        /// <summary>
        /// Added by A D J A Naidu on 06-Jul-2012
        /// This method is used for hosting the multiple web sites pointing to diff data bases in  ISS
        /// </summary>
        ...
        //    //  bool isWebConfigContent = false;

        //    int intIndex = 0;
        //    int intLevelsForRoot = 2;
        //    string strPhysicalApplicationPath = "";
        //    string strRootWebSitePath = "";

        //    string[] strArrayDirectories;
        //    char[] chrArrayDelimeter = new char[] { '\\' };


        //    ConnectionStringSettings constr = ConfigurationManager.ConnectionStrings[1];
        //    strPhysicalApplicationPath = System.Web.Hosting.HostingEnvironment.ApplicationPhysicalPath;
        //    //strPhysicalApplicationPath = @"C:\WHIS-V4-Sites\Services\Wipro.NxG.BillingFacade.Host\bin";
        //    strArrayDirectories = strPhysicalApplicationPath.Split(chrArrayDelimeter);

        //    for (intIndex = 0; intIndex < strArrayDirectories.Length - intLevelsForRoot; intIndex++)
        //    {
        //        strRootWebSitePath += strArrayDirectories[intIndex] + "\\";
        //    }
        ...
        //                connString = this.GetConnectionStringForMaster(MasterVal, strRootWebSitePath);
        //                break;
        //            case 2:
        //                string TranVal = "DBConnectionStringTrans";
        //                connString = this.GetConnectionStringForMaster(TranVal, strRootWebSitePath);
        //                break;
        //            default:
        //                throw new Exception("Database ID Error");
        ...
        //}
        /// <summary>
        /// This method is  used for reading the  constr from web config and used for  multi hosting websites
        /// This method is added by A D J A Naidu on 06-Jul-2012 for hosting the multiple web applications in IIS pointing to
        /// diff data bases.
        /// </summary>
        /// <param name="MasterVal"></param>
        /// <param name="strRootWebSitePath"></param>
        /// <returns></returns>
        ...
        ***************************************************************/
        ...
        private void GetConKeyFromService(int dataBaseID)
        {
            ...
            Dictionary<string, string> hcitKeys = null;
            try
            {
                ...
                {
                    hcitKeys = new Dictionary<string, string>();
                    foreach (string str in request.Headers[HTTP_HEADER].Split(','))
                    {
                        hcitKeys.Add(str.Split(':').GetValue(0).ToString(), str.Split(':').GetValue(1).ToString());
                    }
                }
                ...
                if (hcitKeys != null)
                {
                    ...
                    {
                        case 1:
                            ConnectionStringSettings MasterConnstr = ConfigurationManager.ConnectionStrings[hcitKeys["MasterConKey"]];
                            connString = MasterConnstr.ConnectionString;
                            break;
                        case 2:
                            ConnectionStringSettings TranConnstr = ConfigurationManager.ConnectionStrings[hcitKeys["TranConKey"]];
                            connString = TranConnstr.ConnectionString;
                            break;
                        case 3:
                            ConnectionStringSettings ReportConnstr = ConfigurationManager.ConnectionStrings[hcitKeys["ReportConKey"]];
                            connString = ReportConnstr.ConnectionString;
                            break;
                        default:
        ...
        public static void WritePerfLogText(string Log, string beforeTime, string afterTime, long elapsedTicks, TimeSpan elapsedSpan, string strCommandText, int intParamCount)
        {
            ...
            if (!string.IsNullOrEmpty(strPathName))
            {
                ...
                {
                    sw.WriteLine(startDelimiter + DateTime.Now.ToString(dateTimeFormat) + endDelimiter + startDelimiter + beforeTime + endDelimiter + startDelimiter + afterTime + endDelimiter + startDelimiter + elapsedTicks + endDelimiter + startDelimiter + elapsedSpan + endDelimiter + startDelimiter + strCommandText + endDelimiter + startDelimiter + intParamCount + endDelimiter + Log);
                }
            }
        }
